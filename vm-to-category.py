...
def categoryValuesToAdd(curValues, newValues):
    """ Compares the current list of category values to the one provided. """
    diffValues = list(set(newValues) - set(curValues))
    return diffValues

def categoryValuesToRemove(curValues, newValues):
    """ Compares the current list of category values to the one provided. """
    diffValues = list(set(curValues) - set(newValues))
    return diffValues

def GetVmList(VmName):
    """ Filter VM UUID from VM name
        If multiple VMs with the same name present on the cluster, create a menu to filter by UUID"""
    listVMsPayload = '''{
    "kind": "vm",
    "filter": "vm_name==%s"
    }''' % VmName
...
def GetVmSpec(vmUuid):
    """ Get VM spec from PC filtered by vmUuid """
    jsData2 = json.loads(apiCall(apiBase + endpointVM + vmUuid, auth, None, 'GET').content)
    print('Processing VM: \n',jsData2['spec']['name'],jsData2['metadata']['uuid'])
    return jsData2

def CreatePayload(jsData2):
    """ Create dictionary with category/values data"""
    newPayload = dict()
...
parser = argparse.ArgumentParser(description='''Script to add category and values to VM''', 
                                 usage='''vm-to-gategory.py -u user_with_admin_permissions_here -n vmname_here -i 'PC_IP_or_FQDN_here' -c category_name_here -k category_values_here\n
                                 Category values can be supplied with \",\" as delimiter''')
parser.add_argument('-n', '--vmName', type=str, help='Name of VM', required=True, nargs='+')
...
if not categoryData['entities'] and categoryData['metadata']['length'] ==0:
    ...
    for i in categoryValues:
        if not i in existingCatVal:
            print('Creating missing category Value' + i)
            categoryValueDefinitionPayload = '''{
            "value": "%s",
            "api_version": "3.1.0"
            }''' % i
...
for name in vmName:
    vmUuid=GetVmList(name)
    jsData2=GetVmSpec(vmUuid)
    newPayload=CreatePayload(jsData2)
    ...
    if len(curVMCategories) > 0 and (categoryKey in curVMCategories.keys()):
        valuesToAdd = categoryValuesToAdd(curVMCategories[categoryKey],categoryValues)
        valuesToRemove = categoryValuesToRemove(curVMCategories[categoryKey],categoryValues)
        print(valuesToAdd)
        if len(valuesToAdd) == 0 and len(valuesToRemove) == 0:
            print('nothing to add')
            continue
        elif not len(valuesToAdd) == 0:
            for i in valuesToAdd:
                (curVMCategories[categoryKey]).append(i)
        else:
            for i in valuesToRemove:
                (curVMCategories[categoryKey]).remove(i)
    else:
        curVMCategories[categoryKey] = categoryValues
    
    print(curVMCategories[categoryKey])
